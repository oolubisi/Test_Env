...
async function openTemplatesModal() {
  ...
  body.innerHTML =
    '<div style="text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin"></i> Loading templates...</div>';
...
function renderTemplatesModalBody(body) {
  ...
  html +=
    '<div style="margin-top:8px; font-size:13px; font-weight:800; text-transform:uppercase; color:var(--muted);">Templates</div>';
  html += '<div style="display:flex; gap:6px; flex-wrap:wrap; margin:8px 0;">';
  html +=
    '<button class="action-btn" style="width:auto; padding:6px 12px; font-size:12px; background:var(--card-light); color:var(--text);" onclick="window.refreshTemplatesFromSheet()"><i class="fas fa-sync-alt"></i> Refresh</button>';
  html +=
  ...
  if (customTemplates.length) {
    ...
    html +=
      '<p style="text-align:center; padding:12px; color:var(--muted); font-size:13px;">No templates found. Click Refresh to load from sheet, or Import to add templates.</p>';
  }
...
window.refreshTemplatesFromSheet = async function () {
  ...
  if (body) {
    body.innerHTML =
      '<div style="text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin"></i> Refreshing...</div>';
  }
  try {
    await loadTemplatesFromSheet();
  } catch (e) {
...
function openTakeOffGroupModal(groupId) {
  ...
  if (
    ...
    !String(group.items[0].roomArea || "").startsWith("__GRP__:")
  ) {
    // Legacy singleton — wrap it
    group = {
      groupId: "TO-GRP-" + Date.now(),
      name:
        group.items[0].tradeCategory || group.items[0].description || "Item",
      items: group.items,
      isLegacy: true,
    };
  }
...
async function loadProgressTimelineFeed(forceRefresh = false) {
  ...
  const totalPct = projectLogs.reduce(
    (sum, l) => sum + (Number(l.completionPercentage) || 0),
    0,
  );
  const avgPct = (totalPct / projectLogs.length).toFixed(1);
  if (overallEl) {
    const color =
      Number(avgPct) >= 80
        ? "var(--success)"
  ...
  // Render cards in 2-column grid with edit/delete buttons
  container.innerHTML =
    ...
    projectLogs
      .map((l) => {
        ...
        return `<div class="card" style="text-align:center; display:flex; flex-direction:column; justify-content:center; min-height:80px; position:relative; cursor:pointer;" onclick="window.openModalWithRecord('progress_entry', window.modalRecordCache['${key}'])">
        <div style="position:absolute; top:6px; right:6px; display:flex; gap:4px;">
          <button onclick="event.stopPropagation(); window.deleteProgressLog('${escapeAttr(l.logId)}')" style="background:var(--danger); color:white; border:none; border-radius:50%; width:24px; height:24px; font-size:12px; cursor:pointer; display:flex; align-items:center; justify-content:center;">×</button>
        </div>
        ...
        <span style="font-size:10px; color:var(--muted); margin-top:2px;">${escapeHtml(l.dateRecorded || "")}</span>
      </div>`;
      })
      .join("") +
    `</div>`;
