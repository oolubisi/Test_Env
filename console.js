...
export async function loadProjectConsoleHub(projectId) {
  ...
  if (cSubtotalEl) {
    cSubtotalEl.value =
      (proj.contractSubtotal != null ? Number(proj.contractSubtotal) : 0) || 0;
...
export function toggleContractSubtotalEdit(isEditing) {
  const subtotalEl = document.getElementById("c-meta-contract-subtotal");
  const saveBtn = document.getElementById("contract-subtotal-save-btn");
  if (!subtotalEl) return;
  subtotalEl.readOnly = !isEditing;
  subtotalEl.style.background = isEditing ? "#fff" : "#f5f5f5";
  if (saveBtn) saveBtn.style.display = isEditing ? "block" : "none";
  if (isEditing) subtotalEl.focus();
}

export async function saveProjectContractSubtotal() {
  const btn = document.getElementById("contract-subtotal-save-btn");
  const subtotalEl = document.getElementById("c-meta-contract-subtotal");
  const toggle = document.getElementById("contract-subtotal-edit-toggle");
  const projectId = getCurrentProjectId();
  if (!projectId || !subtotalEl) return;

  const newSubtotal = Number(subtotalEl.value) || 0;
  btn.disabled = true;
  btn.innerText = "Saving...";
  try {
    await callApi("updateProjectContractSubtotal", {
      projectId,
      contractSubtotal: newSubtotal,
    });
    const cache = getCache();
    const proj = cache.projects.find((p) => p.projectId === projectId);
    if (proj) proj.contractSubtotal = newSubtotal;
    setCache(cache);
    btn.innerText = "Save";
    btn.disabled = false;
    if (toggle) toggle.checked = false;
    toggleContractSubtotalEdit(false);
  } catch (e) {
    btn.innerText = "Save";
    btn.disabled = false;
    alert(
      "Failed to save contract subtotal: " + (e.message || "Unknown error"),
    );
  }
}
...
export function switchConsoleSegment(seg) {
  ...
  if (seg === "contract-subtotal") {
    const cSubtotalEl = document.getElementById("c-meta-contract-subtotal");
    const proj = (getCache().projects || []).find(
      (p) => p.projectId === getCurrentProjectId(),
    );
    if (cSubtotalEl && proj)
      cSubtotalEl.value =
        (proj.contractSubtotal != null ? Number(proj.contractSubtotal) : 0) ||
        0;
  }
