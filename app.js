...
// ===== PCR AUTO-SAVE ON PROGRESS CHANGE =====
// Watches #console-progress-overall for DOM changes (set by loadProgressTimelineFeed)
// and automatically saves pcrCompletion + pcrStatus to the sheet.
(function initPcrProgressObserver() {
  let _pcrSaveTimer = null;
  let _lastSavedPct = null;

  function _onOverallChanged() {
    const overallEl = document.getElementById("console-progress-overall");
    if (!overallEl) return;

    // Run updatePcrFields first so the input values are current
    updatePcrFields();
    ...
    if (pct === _lastSavedPct) return;

    // Debounce: wait 800ms after the last DOM change before saving
    clearTimeout(_pcrSaveTimer);
    _pcrSaveTimer = setTimeout(async () => {
      const projectId =
      ...
      const payload = {
        projectId,
        pcrCompletion: Number(pct.toFixed(1)),
      ...
      try {
        ...
        _lastSavedPct = pct;
        // Update cache so the report picks up the new value immediately
        const cache = getCache();
  ...
  // Attach observer once the target element exists.
  // #console-progress-overall may not exist at script load time, so we poll briefly.
  function _attachObserver() {
    ...
    const observer = new MutationObserver(_onOverallChanged);
    observer.observe(overallEl, {
      childList: true,
      subtree: true,
      characterData: true,
    });
