...
// --- PDF ENGINE ---
// --- STANDARDIZED REPORT HEADER / FOOTER ---
      ...
      <table style="width:100%; border-collapse:collapse; border:none; margin:0;">
        <tr style="border:none;">
          <td style="border:none; vertical-align:top; width:55%; padding:0;">
            ${logoHtml}
            <h1 style="margin:0; font-size:20px; font-weight:900; text-transform:uppercase; letter-spacing:1px; color:#000;">${escapeHtml(appSettings.estateName || "FACILITY PRO ESTATE")}</h1>
...
function generateStandardReportFooter() {
  const year = new Date().getFullYear();
  return `
    <div style="border-top: 2px solid #000; margin-top: 25px; padding-top: 10px; page-break-inside: avoid;">
      <table style="width:100%; border-collapse:collapse; border:none; font-size:8px; color:#555;">
        <tr style="border:none;">
          <td style="border:none; width:33%; padding:0; vertical-align:top;">
            <strong style="font-size:9px; color:#000;">${escapeHtml(appSettings.estateName || "Facility Pro")}</strong><br>
            <span style="color:#777;">${escapeHtml(appSettings.estateAddress || "")}</span>
          </td>
          <td style="border:none; width:33%; padding:0; vertical-align:top; text-align:center;">
            <span style="font-size:8px; color:#888;">Page</span>
          </td>
          <td style="border:none; width:33%; padding:0; vertical-align:top; text-align:right;">
            <span style="font-size:8px; color:#888;">&copy; ${year} ${escapeHtml(appSettings.fmName || "Facility Management")}. Confidential.</span>
          </td>
        </tr>
      </table>
      <p style="margin:4px 0 0 0; font-size:7px; color:#999; text-align:center; font-style:italic;">This document is generated by Facility Pro Mobile and is intended solely for authorised internal use.</p>
    </div>
  `;
}
...
function wrapReportContent(contentHtml, reportTitle, reportRef = "") {
  return `
    <div style="font-family: 'Helvetica', 'Arial', sans-serif; color: #000; background: #fff; width: 100%; max-width: 900px; margin: 0 auto; padding: 0; line-height: 1.4; box-sizing: border-box;">
      ${generateStandardReportHeader(reportTitle, reportRef)}
      <div style="min-height: 500px;">
        ${contentHtml}
      </div>
      ${generateStandardReportFooter()}
    </div>
  `;
}
...
async function compileAndDownloadUnifiedPDF(
  ...
  try {
    ...
    const wrappedHtml = wrapReportContent(htmlContent, reportTitle, reportRef);
...
function printSingleWorkOrderDirect(woId, includeAttachments = true) {
  ...
  let invoiceLayoutHtml = `
    <div style="width: 100%; padding: 0; font-family:'Inter', sans-serif; color:#000; background:#fff; box-sizing: border-box;">
  ...
  if (ref && ref.startsWith("WO-")) {
    ...
    if (wo) {
      ...
      finalLayoutHtml += `
        ...
        <div style="width: 800px; padding: 20px 40px; font-family:'Inter', sans-serif; color:#000; background:#fff; box-sizing: border-box;">
          <h3 style="text-transform: uppercase; margin-bottom: 15px; font-size: 16px; font-weight: 900;">[SUPPORTING DOCUMENT] Work Order Authorization Form</h3>
          <table style="width:100%; border-collapse:collapse; margin-bottom:20px; font-size:14px;">
    ...
    if (exr) {
      finalLayoutHtml += `
        <div style="page-break-before: always; height: 0; clear: both;"></div>
        <div style="width: 800px; padding: 20px 40px; font-family:'Inter', sans-serif; color:#000; background:#fff; box-sizing: border-box;">
          <h3 style="text-transform: uppercase; margin-bottom: 15px; font-size: 16px; font-weight: 900;">[SUPPORTING DOCUMENT] Expense Request Form</h3>
          <table style="width:100%; border-collapse:collapse; margin-bottom:20px; font-size:14px;">
...
function compileReportPreview() {
  ...
  // Header is now injected by wrapReportContent for consistency
  ...
  const reportTitle =
    document.getElementById("rep-layout-selector").selectedOptions[0]?.text ||
    "Facility Report";
  const reportRef = generateReportRef("RPT");
  ...
  window.currentReportRef = reportRef;

  const wrappedOut = wrapReportContent(out, reportTitle, reportRef);
  if (viewport) viewport.innerHTML = wrappedOut;
  const printContainer = document.getElementById("report-print-container");
  if (printContainer) printContainer.innerHTML = wrappedOut;
...
function generateApartmentManifestReport() {
  ...
  let html = `
  <div style="font-family: 'Arial', sans-serif; color: #000; background: #fff; padding: 0; width: 100%; margin: 0 auto; line-height: 1.4;">
...
function generateApartmentDossierReport(targetUnitId) {
  ...
  let html = `
  <div style="font-family: 'Arial', sans-serif; color: #000; background: #fff; padding: 0; width: 100%; margin: 0 auto; line-height: 1.4;">
      <h3 style="font-size: 16px; font-weight: 900; text-transform: uppercase; margin: 0 0 15px 0; text-decoration: underline;">Apartment Detailed Dossier &bull; Unit ${escapeHtml(targetUnitId)}</h3>
