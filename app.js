...
function getUnitNumber(u) {
  if (!u) return '';
  const keys = ['apt', 'Apt', 'APT', 'unit', 'Unit', 'UNIT', 'apartment', 'Apartment'];
  for (const key of keys) {
    if (u[key] !== undefined && u[key] !== null) return String(u[key]);
  }
  for (const key in u) {
    if (key.toLowerCase().trim() === 'apt' || key.toLowerCase().trim() === 'unit') return String(u[key]);
  }
  return '';
}
...
async function callApi(action, data = {}) {
  try {
    ...
    if (action.startsWith('get')) {
      ...
      if (backup) {
        try { return JSON.parse(backup); } catch(e) {}
      }
      return []; 
    }

    if (action === 'uploadImage') {
      showToast("Cannot upload photos while offline. Please reconnect.", 'error');
      return null; 
    }
...
async function loadApplicationSettingsData() {
  ...
  if (stored) {
    try {
      ...
      if (parsed && typeof parsed === 'object') appSettings = { ...appSettings, ...parsed };
  ...
  try {
    ...
    if (cloudSettings && typeof cloudSettings === 'object' && (cloudSettings.estateName || cloudSettings.fmName)) {
...
function syncSettingsInputsToUIFields() {
  const fields = ['cfg-estate-name', 'cfg-estate-address', 'cfg-fm-name', 'cfg-fm-address', 'cfg-logo-url'];
  const keys = ['estateName', 'estateAddress', 'fmName', 'fmAddress', 'logoUrl'];
  fields.forEach((id, i) => {
...
function renderGeneratorEfficiencyLogs() {
  ...
  const plantLogs = (cache.utilities || [])
    .filter(u => u && u.type === 'Plant Check' && String(getUnitNumber(u)).includes('GENERATOR'))
...
function renderTotalBalance() {
  ...
  let totalInflow = 0, totalOutflow = 0, pendingInflow = 0, pendingOutflow = 0;
...
function updateDashboardCounters() {
  const tenancyCount = cache.apts ? cache.apts.filter(a => {
    if (!a) return false;
    const status = String(a.status || a.Status || '').toLowerCase();
    const tenant = String(a.tenant || a.Tenant || '').toLowerCase();
    const type = String(a.type || a.Type || '').toLowerCase();
    return status !== 'services' && tenant !== 'services' && type !== 'services';
  }).length : 0;

  const assetCount = cache.assets ? cache.assets.filter(a => {
    if (!a) return false;
    return String(a.status || a.Status || '') !== 'Archived' && String(a.archived || a.Archived || '') !== 'Yes';
  }).length : 0;

  const invCount = cache.inventory ? cache.inventory.filter(i => i && String(i.archived || i.Archived || '') !== 'Yes').length : 0;
  const maintCount = cache.tickets ? cache.tickets.filter(t => t && String(t.status || t.Status || '') !== 'Resolved').length : 0;
  const woCount = cache.workorders ? cache.workorders.filter(w => w && String(w.status || w.Status || '') === 'Pending Approval').length : 0;
  ...
  const ids = ['s-tenancy', 's-asset', 's-inv', 'count-maint', 's-wo', 's-pm-due'];
  const vals = [tenancyCount, assetCount, invCount, maintCount, woCount, pmCombinedSum];
  ids.forEach((id, i) => {
    const el = document.getElementById(id);
...
const filterList = debounce((pageType, query) => {
  const containerMap = {
    'apartments': 'apt-list', 'assets': 'asset-list', 'maintenance': 'maint-list',
    'workorders': 'wo-list', 'inventory': 'inventory-list', 'payments': 'payment-list',
    'staff': 'staff-list', 'vendors': 'vendor-list'
  };
  ...
  if (emptyEl) emptyEl.style.display = visibleCount === 0 && q === '' ? 'block' : 'none';
}, 250);
...
function openRecordRow(type, lookupId) {
  ...
  if (type === 'apartment') match = cache.apts.find(i => i && String(getUnitNumber(i)) === safeLookup);
  if (type === 'asset') match = cache.assets.find(i => i && String(i.tag || i.Tag || i.TAG) === safeLookup);
  if (type === 'maintenance') match = cache.tickets.find(i => i && String(i.ticketId || i.TicketId || i.TICKETID) === safeLookup);
  if (type === 'staff') match = cache.staff.find(i => i && String(i.rowId || i.RowId || i.ROWID) === safeLookup);
  if (type === 'vendor') match = cache.vendors.find(i => i && String(i.rowId || i.RowId || i.ROWID) === safeLookup);
  if (type === 'workorder') match = cache.workorders.find(i => i && String(i.workOrderId || i.WorkOrderId || i.WORKORDERID) === safeLookup);
  if (type === 'inventory') match = cache.inventory.find(i => i && String(i.itemId || i.ItemId || i.ITEMID) === safeLookup);
  if (type === 'payment') match = cache.payments.find(i => i && String(i.paymentId || i.PaymentId) === safeLookup);
  if (type === 'expenserequest') match = cache.expenseRequests.find(i => i && String(i.reqId) === safeLookup);
  if (type === 'cashexpense') match = cache.cashExpenses.find(i => i && String(i.cashId) === safeLookup);
  if (type === 'utility' || type === 'generator') {
    match = cache.utilities.find((i, idx) => i && String(i.rowId || i.id || idx) === safeLookup);
  }
...
function refreshData(p) {
  ...
  callApi(apiCmd, {}).then(data => {
    ...
    if (p === 'assets') { 
      ...
      displayData = displayData.filter(item => item && String(item.status || item.Status || '') !== 'Archived' && String(item.archived || item.Archived || '') !== 'Yes'); 
    }
    if (isMaint) cache.tickets = displayData;
    if (p === 'staff') { 
      cache.staff = displayData; 
      displayData = displayData.filter(item => item && String(item.archived || item.Archived || '') !== 'Yes'); 
    }
    if (p === 'vendors') { 
    ...
    if (p === 'assets') {
      ...
      if (filter && filter.value !== "ALL") {
        displayData = displayData.filter(item => String(getUnitNumber(item)) === filter.value);
      }
    }
    if (isMaint) {
      const filter = document.getElementById('maint-status-filter');
      if (filter && filter.value !== "ALL") {
        displayData = displayData.filter(item => String(item.status || item.Status || '') === filter.value);
      }
    }
    if (p === 'workorders') {
    ...
    const emptyEl = document.getElementById(emptyId);
    if (emptyEl) emptyEl.style.display = displayData.length === 0 ? 'block' : 'none';
  ...
  listEl.innerHTML = displayData.map((item, idx) => {
    ...
    if (p === 'expenserequests') {
      ...
      const submittedPreview = (item.submittedValue || item.SubmittedValue) 
        ...
        : '';
      const statusColor = String(item.status || '') === 'Approved' ? 'var(--success)' : String(item.status || '') === 'Declined' ? 'var(--danger)' : 'var(--warning)';
      ...
      const reasonPreview = item.reason ? `<div style="font-size:13px; color:var(--muted); margin-top:2px;">${escapeHtml(item.reason)}</div>` : '';
      const linkPreview = item.reference ? `<span style="background:var(--card-light); padding:2px 6px; border:1px solid var(--border); border-radius:4px; font-size:10px; margin-left:6px;"><i class="fas fa-link"></i> ${escapeHtml(item.reference)}</span>` : '';
        ...
        <div class="card" onclick="openRecordRow('payment', '${escapeHtml(item.paymentId || item.PaymentId)}')">
          <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:4px;">
            <div>
              ...
              <small style="color:var(--muted); font-weight:700;">ID: ${escapeHtml(item.paymentId || item.PaymentId || '')} | ${bankPreview}${(item.account || item.Account) ? String(item.account || item.Account).padStart(10, '0') : 'N/A'}</small>
            </div>
            <div style="display:flex; flex-direction:column; align-items:flex-end; gap:8px;">
      ...
      const imgSrc = getDirectImageUrl(item.passport || item.Passport) || "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='40' height='40'%3E%3Cpath d='M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z' fill='%23ccc'/%3E%3C/svg%3E";
      return `
      ...
      return `
        <div class="card" onclick="openRecordRow('${itemType}', '${escapeHtml(lookupId)}')" style="border-left: 6px solid ${isPlant ? '#fd7e14' : 'var(--primary)'}; cursor: pointer;">
          <div style="display:flex; justify-content:space-between; align-items:start;">
...
function populateModalInlineImageGalleryPreviews(renderBoxId) {
  ...
  box.innerHTML = currentModalFiles.map((url, idx) => {
    ...
    let content = isPdf 
      ...
      : `<img src="${getDirectImageUrl(url)}" style="width:100%; height:100%; object-fit:cover; border:2px solid var(--text); border-radius:6px; margin:0;" alt="Attachment ${idx+1}">`;
...
function clearAvatarPhotoFrame() {
  currentAvatarPhoto = "";
  const frame = document.getElementById('passport_frame_view');
  if (frame) {
    frame.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' width='50' height='50'%3E%3Cpath d='M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z' fill='%236c757d'/%3E%3C/svg%3E";
  }
  ...
  Array.from(filesList).forEach(file => {
    ...
    if (isPdf) {
      if (file.size > 500 * 1024) {
        showToast(`PDF "${file.name}" exceeds 500KB limit. Skipped.`, 'warning');
        return;
      }
...
function openModal(type, editData = null) {
  ...
  if (type === 'expenserequest') {
    ...
    if (isEdit && (editData.attachments || editData.Attachments)) {
      currentModalFiles = String(editData.attachments || editData.Attachments).split(',').filter(Boolean);
    }
    ...
    submit.onclick = () => {
      ...
      if (!costVal) { showToast("Estimated Cost is required.", 'error'); return; }
  ...
  // --- CASH EXPENSE ---
    ...
    submit.onclick = () => {
      ...
      if (!amtVal) { showToast("Amount is required.", 'error'); return; }
      submit.disabled = true;
      submit.classList.add('loading');
  ...
  // --- APARTMENT ---
    ...
    if (isEdit && (editData.photos || editData.Photos)) {
      currentModalFiles = String(editData.photos || editData.Photos).split(',').filter(Boolean);
    }
    body.innerHTML = `
  ...
  // --- MAINTENANCE ---
    ...
    (cache.staff || []).forEach(s => { 
      ...
      if (isEdit && (editData.tech || editData.Tech) === o.value) o.selected=true; 
      tSel.appendChild(o); 
    });
  ...
  // --- WORK ORDER ---
    ...
    const populateAssets = (unitNum) => {
      ...
      if (!unitNum) return;
      const related = cache.assets.filter(a => a && String(getUnitNumber(a)) === String(unitNum) && String(a.status || a.Status) !== 'Archived');
      related.forEach(a => {
        const o = document.createElement('option');
        o.value = a.tag || a.Tag;
    ...
    (cache.staff || []).forEach(s => { 
      ...
      o.value=`${s.name || s.Name} (${s.role || s.Role})`; 
      o.textContent=`${s.name || s.Name} [Staff]`; 
      if (isEdit && (editData.assigned || editData.Assigned) === o.value) o.selected=true; 
      asSel.appendChild(o); 
    });
    (cache.vendors || []).forEach(v => { 
      if (!v) return;
    ...
    if (isApproved) { 
      ...
      submit.onclick = () => {
        ...
        if (!amtVal) { showToast("Negotiated Value is required.", 'error'); return; }
        submit.disabled = true;
        submit.classList.add('loading');
        callApi(isEdit ? 'updateWorkOrder' : 'saveWorkOrder', { 
          ...
          showToast(isEdit ? 'Work order updated' : 'Work order saved', 'success');
        }).catch(() => {
          submit.disabled = false;
          submit.classList.remove('loading');
        });
  ...
  // --- PAYMENT ---
    ...
    let partyOpts = '';
    (cache.apts || []).forEach(a => { if (a && a.tenant && a.tenant.toLowerCase() !== 'services') partyOpts += `<option value="${escapeHtml(a.tenant)}">`; });
    (cache.staff || []).forEach(s => { if (s && s.name) partyOpts += `<option value="${escapeHtml(s.name)}">`; });
    (cache.vendors || []).forEach(v => { if (v && v.company) partyOpts += `<option value="${escapeHtml(v.company)}">`; });
    ...
    const approvedWOs = (cache.workorders || []).filter(w => w && String(w.status) === 'Approved');
    ...
    body.innerHTML = `
      ...
      <input list="party_list" id="p_party" value="${isEdit ? escapeHtml(editData.party || '') : ''}" placeholder="Type or select from list..." ${largeInput} ${disabledState}>
      ...
      <label ${labelStyle}>Classification Note</label><input id="p_type" value="${isEdit ? escapeHtml(editData.type || '') : ''}" placeholder="e.g. Rent, Vendor Payment" ${largeInput} ${disabledState}>
      <label ${labelStyle}>Reason / Justification</label><textarea id="p_reason" rows="2" placeholder="Describe the transaction..." ${largeInput} ${disabledState}>${isEdit ? escapeHtml(editData.reason || '') : ''}</textarea>
      ...
      <div style="background:#f8f9fa; padding:15px; border-radius:8px; border:1px solid #dee2e6; margin:15px 0;">
        ...
        <input id="p_amount" type="number" required value="${isEdit ? escapeHtml(editData.amount || '') : ''}" placeholder="Current payment amount" ${largeInput} style="border-color:var(--primary); border-width:3px;" ${disabledState}>
    ...
    document.getElementById('p_party').addEventListener('change', (e) => {
      ...
      const vendorMatch = cache.vendors.find(v => v && (v.company || v.Company) === selectedParty);
      ...
      const staffMatch = cache.staff.find(s => s && (s.name || s.Name) === selectedParty);
    ...
    submit.onclick = () => {
      ...
      if (!amtVal) { showToast("Amount to Pay is required.", 'error'); return; }

      let accVal = document.getElementById('p_account').value;
      if (accVal) accVal = String(accVal).padStart(10, '0');
      if (accVal && accVal.length !== 10) { showToast("Account Number must be exactly 10 digits.", 'error'); return; }
  ...
  // --- GENERATOR ---
    ...
    submit.onclick = () => {
      ...
      callApi(isEdit ? 'updateUtility' : 'saveUtility', { 
        ...
        closeModal(); 
        refreshData('utilities'); 
        showToast(isEdit ? 'Plant log updated' : 'Plant log saved', 'success');
      }).catch(() => {
        submit.disabled = false;
        submit.classList.remove('loading');
      });
  ...
  // --- STAFF ---
    ...
    document.getElementById('st_pass_uploader').onchange = (e) => {
      ...
      r.onload = async (evt) => {
        ...
        callApi('uploadImage', { base64: comp, name: "pass_" + uniqueId + ".jpg" }).then(res => { 
          if (res && res.url) { 
            currentAvatarPhoto = res.url; 
            document.getElementById('passport_frame_view').src = getDirectImageUrl(res.url); 
            document.getElementById('p_avatar_remove_btn').style.display='flex'; 
          } 
        });
      }; 
    ...
    submit.onclick = () => {
      ...
      if (!p1 || p1.length !== 11) { showToast("Phone 1 must be exactly 11 digits.", 'error'); return; }
      if (p2 && p2.length !== 11) { showToast("Phone 2 must be exactly 11 digits if provided.", 'error'); return; }
      submit.disabled = true;
      submit.classList.add('loading');
      ...
      callApi(isEdit?'updateStaff':'saveStaff', { 
        ...
        account: String(document.getElementById('st_account').value).padStart(10, '0'),
  ...
  // --- VENDOR ---
    ...
    document.getElementById('v_pass_uploader').onchange = (e) => {
      ...
      r.onload = async (evt) => {
        ...
        callApi('uploadImage', { base64: comp, name: "vpass_" + uniqueId + ".jpg" }).then(res => { 
          if (res && res.url) { 
            currentAvatarPhoto = res.url; 
            document.getElementById('vendor_frame_view').src = getDirectImageUrl(res.url); 
            document.getElementById('p_avatar_remove_btn').style.display='flex'; 
          } 
        });
      }; 
    ...
    submit.onclick = () => {
      ...
      callApi(isEdit?'updateVendor':'saveVendor', { 
        ...
        account: String(document.getElementById('v_account').value).padStart(10, '0'),
  ...
  // Focus first input
  setTimeout(() => {
    const firstInput = body.querySelector('input:not([disabled]), select:not([disabled]), textarea:not([disabled])');
...
async function compileAndDownloadUnifiedPDF(sourceElement, attachmentUrls = [], filename = 'Facility_Report') {
  ...
  try {
    ...
    let result;
    const text = await response.text();
    try { result = JSON.parse(text); } catch(e) { throw new Error('Invalid PDF response'); }

    if (result.status !== 'success') throw new Error(result.message || 'PDF generation failed');
        ...
        try {
          ...
          if (fileData && fileData.status === 'success' && fileData.base64) {
            ...
            if (fileData.mimeType === 'application/pdf') {
              ...
              let img;
              if (fileData.mimeType === 'image/png') img = await masterPdf.embedPng(bytes);
...
function printSingleWorkOrderDirect(woId, includeAttachments = true) {
  const orderItem = cache.workorders.find(w => w && String(w.workOrderId || w.WorkOrderId) === woId); 
  if (!orderItem) { showToast('Work order not found', 'error'); return; }
  const unitId = getUnitNumber(orderItem);

  let assetInfo = "N/A";
  if (orderItem.asset || orderItem.Asset) {
    const ast = cache.assets.find(a => a && String(a.tag || a.Tag) === String(orderItem.asset || orderItem.Asset));
  ...
  if (includeAttachments && (orderItem.attachments || orderItem.Attachments)) {
    attachmentsArr = String(orderItem.attachments || orderItem.Attachments).split(',').filter(Boolean);
  }
...
function printSingleExpenseRequestDirect(reqId) {
  const orderItem = cache.expenseRequests.find(w => w && String(w.reqId || w.ReqId) === reqId); 
  if (!orderItem) { showToast('Request not found', 'error'); return; }
  const unitId = getUnitNumber(orderItem);
...
function printSingleCashExpenseDirect(cashId) {
  const orderItem = cache.cashExpenses.find(w => w && String(w.cashId || w.CashId) === cashId); 
  if (!orderItem) { showToast('Expense not found', 'error'); return; }
  const unitId = getUnitNumber(orderItem);
...
function printSinglePaymentDirect(paymentId) {
  const orderItem = cache.payments.find(p => p && String(p.paymentId || p.PaymentId) === paymentId); 
  if (!orderItem) { showToast('Payment not found', 'error'); return; }
  ...
  if (ref && ref.startsWith('WO-')) {
    const wo = cache.workorders.find(w => w && String(w.workOrderId || w.WorkOrderId) === ref);
    if (wo) {
      let assetInfo = "N/A";
      if (wo.asset || wo.Asset) {
        const ast = cache.assets.find(a => a && String(a.tag || a.Tag) === String(wo.asset || wo.Asset));
      ...
      if (wo.attachments || wo.Attachments) combinedAttachmentsArray.push(...String(wo.attachments || wo.Attachments).split(',').filter(Boolean));
    }
  } else if (ref && ref.startsWith('EXR-')) {
    const exr = cache.expenseRequests.find(r => r && String(r.reqId || r.ReqId) === ref);
    if (exr) {
      finalLayoutHtml += `
        <div style="page-break-before: always; height: 0; clear: both;"></div>
        <div style="width: 800px; padding: 20px 40px; font-family:'Inter', sans-serif; color:#000; background:#fff; box-sizing: border-box;">
      ...
      if (exr.attachments || exr.Attachments) combinedAttachmentsArray.push(...String(exr.attachments || exr.Attachments).split(',').filter(Boolean));
    }
  }
...
function compileReportPreview() {
  ...
  if (layout === "detailed_profile") {
    ...
    if (!unit) { showToast("Please select a unit.", 'warning'); return; }
  ...
  if (layout === "occupancy_report") {
    ...
    if (!startRaw || !endRaw) { showToast("Please select a date range.", 'warning'); return; }

    const startDate = new Date(startRaw); 
    const endDate = new Date(endRaw);
    const gens = (cache.utilities || []).filter(u => u && (String(u.type || '').includes('Plant') || String(u.type || '').includes('Generator')));
    ...
    const assets = (cache.assets || []).filter(a => a && String(a.status || '') !== 'Archived');
    ...
    if (!reportDate) { showToast("Please select a date.", 'warning'); return; }

    const dailyTickets = (cache.tickets || []).filter(t => t && t.date === reportDate);
    const closedTickets = dailyTickets.filter(t => t && t.status === 'Completed');
    ...
    const apts = cache.apts || [];
    const occupied = apts.filter(a => a && String(a.status || '').toLowerCase() === 'occupied').length;
    const occPercentage = apts.length > 0 ? Math.round((occupied / apts.length) * 100) : 0;

    let mInflow = 0; let mOutflow = 0;
    (cache.payments || []).filter(p => p && p.date && p.date.startsWith(monthStr)).forEach(p => mInflow += parseFloat(p.amount || 0));
    (cache.cashExpenses || []).filter(c => c && c.date && c.date.startsWith(monthStr)).forEach(c => mOutflow += parseFloat(c.amount || 0));
...
function generateApartmentManifestReport() {
  ...
  let apartments = [...(cache.apts || [])].filter(a => a && String(a.type || '').toLowerCase() !== 'services');
  ...
  apartments.forEach(apt => {
    ...
    const unitAssets = (cache.assets || []).filter(a => a && String(getUnitNumber(a)) === String(getUnitNumber(apt)) && String(a.status || '') !== 'Archived');
...
function generateApartmentDossierReport(targetUnitId) {
  ...
  const apt = (cache.apts || []).find(a => a && String(getUnitNumber(a)) === String(targetUnitId));
  if (!apt) { showToast("Apartment not found.", 'error'); return; }
  ...
  const unitAssets = (cache.assets || []).filter(a => a && String(getUnitNumber(a)) === String(targetUnitId) && String(a.status || '') !== 'Archived');
