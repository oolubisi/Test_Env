...
📍 ${gps}`
    ...
    if (isEdit) {
      ...
      groupItems = [
        { description: "", quantity: "", unit: "", scopeNotes: "" },
      ];
    }
    ...
    if (isEdit) {
      document.getElementById("t_delete_btn").onclick = () => {
        if (confirm("Delete this entire group?")) {
          ...
          Promise.all(
            toDelete.map((id) =>
              callApi("deleteTakeOffItem", { itemId: id }).catch(() => {}),
            ),
          ).then(() => {
            closeModal();
            loadTakeOffListings(true);
    ...
    submit.onclick = async () => {
      ...
      const rows = document.querySelectorAll(
        "#to_group_items_body tr.to-group-line",
      );
      const newItems = [];
      ...
      for (const item of newItems) {
        const payload = {
          ...
          beforePhotoUrl: photoUrl,
          scopeNotes:
            item.scopeNotes + (gps !== "GPS Unavailable" ? "\n📍 " + gps : ""),
        };
        try {
          await callApi(
            item.existing ? "updateTakeOffItem" : "saveTakeOffItem",
            payload,
          );
          saved++;
        } catch (e) {
    ...
    const uniqueId = isEdit ? editData.logId : "LOG-" + Date.now();
    title.innerText = isEdit ? "Edit Progress" : "Log Progress";
    body.innerHTML = `<label ${labelStyle}>Trade</label><input id="l_trade" value="${escapeAttr(isEdit ? editData.tradeCategory : "")}" ${largeInput}><label ${labelStyle}>Completion %</label><input id="l_pct" type="number" min="0" max="100" step="1" placeholder="Enter 0-100" value="${escapeAttr(isEdit ? editData.completionPercentage : "")}" oninput="this.value=this.value.replace(/[^0-9]/g,'');if(this.value>100)this.value=100;if(this.value<0)this.value=0;" ${largeInput}><label ${labelStyle}>Comments</label><textarea id="l_comm" rows="3" ${largeInput}>${escapeHtml(isEdit ? editData.commentNarrative : "")}</textarea><div id="progressAttachmentsPreviews" class="modal-preview-grid" style="display:none;"></div><label class="icon-upload-label"><i class="fas fa-paperclip"></i><input type="file" id="l_photo" accept="image/*" multiple style="display:none"></label>${isEdit ? `<button class="action-btn" id="l_delete_btn" style="background:var(--danger); margin-top:10px;">Delete</button>` : ""}`;
    if (isEdit && editData.progressPhotoUrl) {
      currentModalFiles = splitAttachments(editData.progressPhotoUrl);
      if (currentModalFiles.length)
        populateModalInlineImageGalleryPreviews("progressAttachmentsPreviews");
    }
